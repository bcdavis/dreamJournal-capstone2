...
export const FirebaseProvider = (props) => {
    ...
    const provider = new firebase.auth.GoogleAuthProvider(); // lets us use google login credentials
    ...
    const register = (userProfile, password) => {
        return firebase.auth().createUserWithEmailAndPassword(userProfile, password)
            .then(savedUserProfile => {
                console.log("savedU", savedUserProfile);
                sessionStorage.setItem("userProfile", JSON.stringify(savedUserProfile.user))
                setIsLoggedIn(true);
    ...
    // you return a context provider, allow any child elements to access them
